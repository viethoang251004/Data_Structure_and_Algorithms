...
public class RatingManagement {
    ...
    private ArrayList<Movie> loadMovies(String moviePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(moviePath))) {
            ...
            String line;
            // Bo dong dau tien(header)
            // reader.readLine() doc mot dong tu BufferedReeader nhung gia tri cua dong nay
            // duoc bo qua vi dong dau tien trong file csv la tieu de, sau do doc cac dong
            // con lai
            reader.readLine();
            // tao mot vong lap while voi dieu kien la doi tuong line duoc gan gia tri mot
            // dong du lieu tu BufferedReader(file movies.csv) va line khac null
            while ((line = reader.readLine()) != null) {
                // tao mot mang parts voi datatype la String gom cac phan tu duoc phan cach bang
                // dau phay lay tu file movies.csv
                String[] parts = line.split(",");
                // tao mot doi tuong moi ten la id voi datatype la int va gan cho no phan tu dau
                // tien cua mang voi method ep kieu sang kieu int ket hop voi method trim() de
                // loai bo khoang trang o 2 dau cua chuoi
                int id = Integer.parseInt(parts[0].trim());
                // tao mot doi tuong moi ten la name voi datatype la String va gan cho no phan
                // tu thu hai cua mang voi method trim() de loai bo khoang trang o 2 dau cua
                // chuoi
                String name = parts[1].trim();
                // tạo một đối tượng ArrayList<String> tên là genres voi tham so truyen vao duoc
                // su dung method asList cua class Arrays trong package java.util voi phan tu
                // thu ba cua movies.csv ket hop voi method trim() de loai bo khoang trang o 2
                // dau cua chuoi va method split() duoc phan cach bang dau - lay tu file
                // movies.csv
                ArrayList<String> genres = new ArrayList<>(Arrays.asList(parts[2].trim().split("-")));
                // tao mot doi tuong moi ten la movie voi datatype la Movie va truyen vao 3 tham
                // so do la id(ma so bo phim), name(ten bo phim), genres(danh sach cac the loai
                // bo phim) cua method Movie trong Movie.java
                Movie movie = new Movie(id, name, genres);
                // su dung method add() cua class ArrayList trong package java.util de them doi
                // tuong movie vao danh sach phim(movieList)
    ...
    private ArrayList<User> loadUsers(String userPath) {
        ...
        // tao cap try catch de truy xuat loi ra man hinh cho chung ta dễ nhận diện và
        // fix
        // trong lenh try tao mot doi tuong reader voi datatype la BufferedReader cua
        // package java.io voi tham so dau vao la userPath duoc truyen vao method cua
        // FileReader cua lop FileReader trong package java.io
        try (BufferedReader reader = new BufferedReader(new FileReader(userPath))) {
            ...
            reader.readLine();
            // tao mot vong lap while voi dieu kien la doi tuong line duoc gan gia tri mot
            // dong du lieu tu BufferedReader(file users.csv) va line khac null
            while ((line = reader.readLine()) != null) {
                // tao mot mang parts voi datatype la String gom cac phan tu duoc phan cach bang
                // dau phay lay tu file users.csv
                String[] parts = line.split(",");
                // tao mot doi tuong moi ten la id voi datatype la int va gan cho no phan tu dau
                // tien cua mang voi method ep kieu sang kieu int ket hop voi method trim() de
                // loai bo khoang trang o 2 dau cua chuoi
